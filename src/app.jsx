...
function TapDon({ usr, onDone, notify }) {
  const [mode, sMode] = useState(null);            // null | "unique" | "monthly"
  const [step, sStep] = useState("form");           // form | tap | pay | processing | success
  const [fm, sFm] = useState({ fn: "", ln: "", em: "", amt: 25, cst: "", adr: "", sx: "", ph: "" });
  const [pm, sPm] = useState("carte");              // carte | cheque
  ...
  const back = () => { sEr(""); if (step === "form") { sMode(null); } else { sStep(mode === "unique" ? "form" : step === "pay" ? "form" : "pay"); } };
  const resetAll = () => { sFm({ fn: "", ln: "", em: "", amt: 25, cst: "", adr: "", sx: "", ph: "" }); sCd({ num: "", exp: "", cvc: "" }); sBk({ tr: "", inst: "", acc: "" }); sConsent(false); sEr(""); sStep("form"); sMode(null); if (onDone) onDone(); };
  ...
  const payM = async () => {
    ...
    sEr(""); sStep("processing");
    const t = now(); const id = "mon-" + usr.id + "-" + Date.now();
    try {
      await Promise.all([
        db.post("monthly_donations", [{ id, first_name: fm.fn.trim(), last_name: fm.ln.trim(), address: fm.adr.trim(), email: fm.em.trim(), gender: fm.sx || null, phone: fm.ph.trim(), amount: amt, payment_method: pm, card_last4: pm === "carte" ? digits(cd.num).slice(-4) : null, bank_transit: pm === "cheque" ? digits(bk.tr) : null, bank_institution: pm === "cheque" ? digits(bk.inst) : null, bank_account: pm === "cheque" ? digits(bk.acc) : null, ppa_consent: pm === "cheque" ? consent : false, status: "active", collector_id: usr.id, collector_name: usr.name, team: usr.team, date: td, time: t }]),
        db.post("donations", [{ id, collector_id: usr.id, collector_name: usr.name, team: usr.team, donor: full(), amount: amt, type: "Mensuel", date: td, time: t }]),
      ]);
  ...
  return <div style={{ maxWidth: 530, margin: "0 auto" }}>
    ...
    {mode === "unique" && step === "tap" && <div className="asi" style={{ ...card, overflow: "hidden", boxShadow: "0 12px 44px rgba(45,43,107,.16)" }}>
      ...
      <div style={{ padding: "34px 26px 26px", textAlign: "center" }}>
        ...
        <p style={{ color: C.t3, fontSize: 11.5, fontWeight: 600, marginBottom: 24, display: "inline-flex", alignItems: "center", gap: 5, background: C.bl, padding: "5px 13px", borderRadius: 14 }}><Ic d={ic.lk} sz={11} c={C.t3} />Terminal Square — sera connecté prochainement</p>
        {eBox}
        <button onClick={payU} className="bt" style={{ width: "100%", padding: 17, borderRadius: 15, border: "none", background: `linear-gradient(135deg,${C.rd},${C.rk})`, color: "#fff", fontSize: 15.5, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", gap: 9 }}><Ic d={ic.tp} sz={17} c="#fff" />Présenter la carte (simulation)</button>
    ...
    {mode === "monthly" && step === "pay" && <div className="asi" style={{ ...card, overflow: "hidden", boxShadow: "0 12px 44px rgba(45,43,107,.16)" }}>
      ...
      <div style={{ padding: 24 }}>
        ...
        {pm === "carte" && <div style={{ background: C.bg, border: "1.5px solid " + C.bd, borderRadius: 15, padding: 17, marginBottom: 18 }}>
          ...
          <p style={{ color: C.t3, fontSize: 11, marginTop: 11, textAlign: "center", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}><Ic d={ic.lk} sz={11} c={C.t3} />La carte sera tokenisée par Square — aucun numéro conservé</p>
        </div>}
    ...
    {step === "success" && <div className="asi" style={{ ...card, padding: "42px 30px", textAlign: "center" }}>
      ...
      {mode === "monthly" && <p style={{ color: C.t2, fontSize: 14.5, fontWeight: 500 }}>Don mensuel de <strong style={{ color: C.nv, fontFamily: Fn.m }}>{amt}$/mois</strong> activé ({pm === "carte" ? "carte ····" + digits(cd.num).slice(-4) : "prélèvement bancaire"})</p>}
